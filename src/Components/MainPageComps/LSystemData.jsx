...
function LsysData({ onGenerate }) {
  ...
  const ruleRefs = useRef([
    { key: React.createRef(), rule: React.createRef() },
  ]);
  ...
  const handleGenerateClick = () => {
    ...
    const width =
      widthRef.current.value == null || widthRef.current.value <= 0
        ? 1
        : widthRef.current.value;
    const color = colorRef.current.value;
    const angle =
      angleRef.current.value == null || angleRef.current.value <= 0
        ? 90
  ...
  return (
    <div className="grid grid-cols-2 bg-[rgba(163,163,163,0.23)] rounded-lg shadow-custom backdrop-filter[blur(16.7px)] p-1 pt-1">
      ...
      <div className="grid grid-cols-3 col-span-2 sm:p-1 gap-2 pl-2 ">
